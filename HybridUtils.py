...
def mysql_query(object_with_mysql_connection, *args, **kw): # Handles a MySQL query with reconnection
    if not object_with_mysql_connection.mysql_connection: # For instance, if it's set to None so that it gets lazily constructed only when needed
        object_with_mysql_connection.mysql_connection = connect_mysql()
    
    d = object_with_mysql_connection.mysql_connection.runQuery(*args)

    def handle_failure(failure):
        print "CRITICAL HybridUtils.mysql_query.handle_failure got failure %s" % str(failure)
        failure.trap(OperationalError, ProgrammingError)

        error_strings = [
                """(1017, "Can't find file: './hybridcluster/dbases.frm' (errno: 13)")""",
                """(1017, "Can't find file: './hybridcluster/mailalias_destinations.frm' (errno: 13)")""",
                """(1017, "Can't find file: './hybridcluster/mailaliases.frm' (errno: 13)")""",
                """(1017, "Can't find file: './hybridcluster/mailboxes.frm' (errno: 13)")""",
                """(1017, "Can't find file: './hybridcluster/websites.frm' (errno: 13)")""",
                """(1045, 'Access denied for user - no such database, sorry.')""",
                """(1049, "Unknown database 'hybridcluster'")""",
                """(1146, "Table 'hybridcluster.dbases' doesn't exist")""",
                """(1146, "Table 'hybridcluster.mailalias_destinations' doesn't exist")""",
                """(1146, "Table 'hybridcluster.mailaliases' doesn't exist")""",
                """(1146, "Table 'hybridcluster.mailboxes' doesn't exist")""",
                """(1146, "Table 'hybridcluster.websites' doesn't exist")""",
                ...
                """(1017, "Can't find file: 'dbases' (errno: 2)")""",
                """(1017, "Can't find file: 'websites' (errno: 2)")""",
                """(1017, "Can't find file: 'mailboxes' (errno: 2)")""",
                """(1017, "Can't find file: 'mailaliases' (errno: 2)")""",
                """(1017, "Can't find file: 'mailalias_destinations' (errno: 2)")""",
        ]

        catch_this_failure = False
        for error_string in error_strings:
            if error_string in str(failure):
                catch_this_failure = True

        if catch_this_failure:
            tries = kw.get("tries", 1)
            delay = 2**tries
            print "CRITICAL Caught %s, sleeping for %i seconds" % (str(failure), delay)
            # Wait and then reconnect to MySQL
            d = sleep(delay)
            def after_sleep(data):
                print "CRITICAL Attempting to reconnect to MySQL for %s" % str(args)
                object_with_mysql_connection.mysql_connection.close()
                object_with_mysql_connection.mysql_connection = connect_mysql()
                # Reconnect and recurse
                kw['tries'] = tries + 1
                return mysql_query(object_with_mysql_connection, *args, **kw)
            d.addCallback(after_sleep)
            return d
        else:
            return failure # Some other failure, such as a genuine programming error, which should cause an exception.
...
def connect_mysql():
    import secrets
    return txmysql.blah("MySQLdb", host="127.0.0.1", db='hybridcluster', user='hybridcluster', passwd=secrets.MYSQL_PASS)
